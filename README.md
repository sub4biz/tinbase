...
| Cron | `cron.*` | Scheduled jobs, drop-in with pg_cron's API — `cron.schedule(name, '*/5 * * * *', 'sql')` (also the `'N seconds'` form), `cron.unschedule(...)`, and the `cron.job` / `cron.job_run_details` tables. An in-process scheduler runs due jobs. **Differs from hosted pg_cron:** schedules evaluate in the server's **local timezone** (hosted pg_cron runs in UTC); jobs run only while tinbase is up; the command runs with service-role privileges (RLS bypassed); and there's **no `pg_net`**, so a job can't make an HTTP request — schedule SQL, and use Database Webhooks for change-driven HTTP. No extension |
